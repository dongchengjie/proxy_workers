...
export default {
  enable: true,
  getFileName: () => 'fromzero.yaml',
  getContent: async () => {
    try {
      const html = await axios.get('https://blues2022.blogspot.com/').then(response => response.data);
      const regex = /<h2 class='entry-title'>.*<a\s*href='(.*?)'/;
      const match = regex.exec(html);
      const link = match ? match[1] : null;
      logger.info('fromzero链接: ' + link);
      ...
      logger.info('fromzero加密串: ' + encrypted);
